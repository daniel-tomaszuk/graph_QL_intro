...
from f2f_Q3_2019.settings import N2YO_API_KEY_V1
from f2f_Q3_2019.settings import N2YO_BASE_URL_V1
...
NORAD_IDS_TO_FIND = [ISS_NORAD_ID]
...
def get_satellite_positions():
    ...
    if response.status_code == status.HTTP_200_OK and "error" not in response_data:
        ...
        if satellite_info and satellite_position:
            ...
            satellite_data = dict(name=satellite_info["satname"], norad_id=norad_id,)
            satellite, _ = Satellite.objects.get_or_create(
                norad_id=norad_id, defaults=satellite_data
            )
